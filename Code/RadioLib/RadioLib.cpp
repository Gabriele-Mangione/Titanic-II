...
bool RFM69::initialize(uint8_t freqBand, uint16_t nodeID, uint8_t networkID) {
    ...
    uint8_t timeout = 50;
    do writeReg(REG_SYNCVALUE1, 0xAA);
    while (readReg(REG_SYNCVALUE1) != 0xaa && millis() - start < timeout);
    start = millis();
    do writeReg(REG_SYNCVALUE1, 0x55);
...
ISR_PREFIX void RFM69::isr0() {
    ...
    _instance->select();
    _instance->_spi->transfer(REG_FIFO & 0x7F);
    PAYLOADLEN = _instance->_spi->transfer(0);
    PAYLOADLEN = PAYLOADLEN > 66 ? 66 : PAYLOADLEN;  // precaution
    TARGETID = _instance->_spi->transfer(0);
    SENDERID = _instance->_spi->transfer(0);
    uint8_t CTLbyte = _instance->_spi->transfer(0);
    TARGETID |= (uint16_t(CTLbyte) & 0x0C) << 6;  // 10 bit address (most significant 2 bits stored in bits(2,3) of CTL byte
    SENDERID |= (uint16_t(CTLbyte) & 0x03) << 8;  // 10 bit address (most sifnigicant 2 bits stored in bits(0,1) of CTL byte

    if (!(_instance->_spyMode || TARGETID == _instance->_address || TARGETID == RF69_BROADCAST_ADDR)  // match this node's address, or broadcast address or anything in spy mode
        || PAYLOADLEN < 3)                                                      // address situation could receive packets that are malformed and don't fit this libraries extra fields
    {
        PAYLOADLEN = 0;
        _instance->unselect();
    ...
    _instance->timer = millis();

    DATALEN = PAYLOADLEN - 3;
    ACK_RECEIVED = CTLbyte & RFM69_CTL_SENDACK;  // extract ACK-received flag
    ACK_REQUESTED = CTLbyte & RFM69_CTL_REQACK;  // extract ACK-requested flag
    uint8_t _pl = _instance->_powerLevel;                   // interruptHook() can change _powerLevel so remember it
    _instance->interruptHook(CTLbyte);                      // TWS: hook to derived class interrupt function

    for (uint8_t i = 0; i < DATALEN; i++) DATA[i] = _instance->_spi->transfer(0);

    DATA[DATALEN] = 0;  // add null at end of string // add null at end of string
    _instance->unselect();
    _instance->setMode(RF69_MODE_RX);
    if (_pl != _instance->_powerLevel) _instance->setPowerLevel(_instance->_powerLevel);  // set new _powerLevel if changed
